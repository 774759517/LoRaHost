...
void SX1276LoRaInit( void )
{
    ...
    SX1276LR->RegLna = RFLR_LNA_GAIN_G1;//接收机增益
...
#endif
        SX1276LoRaSetPAOutput( RFLR_PACONFIG_PASELECT_PABOOST ); //选择 PA_BOOST 管脚输出信号  RFLR_PACONFIG_PASELECT_PABOOST
...
uint32_t SX1276LoRaProcess( void )
{
    ...
    case RFLR_STATE_RX_INIT:
        ...
        SX1276Write( REG_LR_IRQFLAGSMASK, SX1276LR->RegIrqFlagsMask );			//清中断
        ...
        if( LoRaSettings.FreqHopOn == true )
        {
            ...
            SX1276LoRaSetRFFrequency( HoppingFrequencies[SX1276LR->RegHopChannel & RFLR_HOPCHANNEL_CHANNEL_MASK] );//设置发射频率
        }
        else
        {
        ...
        SX1276WriteBuffer( REG_LR_DIOMAPPING1, &SX1276LR->RegDioMapping1, 2 );					//设置IO映射
...
//				{
//					
//					SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_RXDONE  );//清除标志
//					RFLRState = RFLR_STATE_RX_DONE;; //发送完成更新状态机
//				}
        break;
    ...
    case RFLR_STATE_TX_INIT://发送初始化
        ...
        else
        {
            SX1276LR->RegIrqFlagsMask = RFLR_IRQFLAGS_RXTIMEOUT |
                                        ...
                                        RFLR_IRQFLAGS_FHSSCHANGEDCHANNEL |
                                        RFLR_IRQFLAGS_CADDETECTED;
            SX1276LR->RegHopPeriod = 0;//频率跳变的符号周期 0代表关闭
        }
        SX1276Write( REG_LR_HOPPERIOD, SX1276LR->RegHopPeriod );
        ...
        SX1276Write( REG_LR_PAYLOADLENGTH, SX1276LR->RegPayloadLength );//负载长度最大值
        
        SX1276LR->RegFifoTxBaseAddr = 0x00; // Full buffer used for Tx
        SX1276Write( REG_LR_FIFOTXBASEADDR, SX1276LR->RegFifoTxBaseAddr );//FIFO基地址

        SX1276LR->RegFifoAddrPtr = SX1276LR->RegFifoTxBaseAddr;
        SX1276Write( REG_LR_FIFOADDRPTR, SX1276LR->RegFifoAddrPtr );//FIFO数据缓冲区中SPI接口地址指针
        
        // Write payload buffer to LORA modem
        SX1276WriteFifo( RFBuffer, SX1276LR->RegPayloadLength );//数据写入缓冲区
        ...
        SX1276LoRaSetOpMode( RFLR_OPMODE_TRANSMITTER );//发送数据
        RFLRState = RFLR_STATE_TX_RUNNING;//更新状态机
        break;
...
//				if((temp&0x08)==0x08)
        if( DIO0 == 1 ) // TxDone
        {
            ...
            SX1276Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_TXDONE  );
					RFLRState = RFLR_STATE_TX_DONE; //发送完成更新状态机  
        }
//				if((temp&0x02)==0x02)
        if( DIO2 == 1 ) // FHSS Changed Channel
        {
